            ...
            for binlog_event in stream:
                ...
                if isinstance(binlog_event, QueryEvent) and not self.only_dml:
                    sql = concat_sql_from_binlog_event(cursor=cursor, binlog_event=binlog_event,
                                                       flashback=self.flashback, no_pk=self.no_pk, thread_id=self.thread_id)
                    if sql:
                        print(sql)
                elif is_dml_event(binlog_event) and event_type(binlog_event) in self.sql_type:
                    for row in binlog_event.rows:
                        sql = concat_sql_from_binlog_event(cursor=cursor, binlog_event=binlog_event, no_pk=self.no_pk,
                                                           row=row, flashback=self.flashback, e_start_pos=e_start_pos, thread_id=self.thread_id)
                        if self.flashback:
                            f_tmp.write(sql + '\n')
                        else:
                            print(sql)
...
if __name__ == '__main__':
    ...
    binlog2sql = Binlog2sql(connection_settings=conn_setting, thread_id=args.thread_id, start_file=args.start_file, start_pos=args.start_pos,
